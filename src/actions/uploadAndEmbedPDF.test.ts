...
vi.mock("@supabase/supabase-js", () => {
  ...
  getPublicUrlMock = vi
    .fn()
    .mockReturnValue({ data: { publicUrl: "https://supabase.test/file.pdf" } });
...
vi.mock("openai", () => {
  embeddingsCreateMock = vi
    .fn()
    .mockResolvedValue({ data: [{ embedding: [1, 2, 3] }] });
  chatCreateMock = vi.fn().mockResolvedValue({
...
describe("uploadAndEmbedPDF", () => {
  ...
  it("uploads file and stores embedding", async () => {
    ...
    const { uploadAndEmbedPDF } = await import("@/actions/uploadAndEmbedPDF");
    const formData = new FormData();
    const file = new File(["dummy"], "test.pdf", { type: "application/pdf" });
    Object.defineProperty(file, "arrayBuffer", {
      value: async () => Buffer.from("PDF content"),
    });
